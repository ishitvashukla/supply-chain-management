...
2. Render reads `render.yaml`, so the settings below are pre-filled — confirm them:
   - Runtime **Node**, plan **Free**
   - Build: `npm run install:all && npm run build`
   - Start: `npm start`
   - Health check: `/api/v1/health`
3. Add the one secret it cannot generate: **`MONGO_URI`** (your Atlas connection
   string). `JWT_SECRET` and `JWT_REFRESH_SECRET` are generated by Render.
4. In **Atlas → Network Access**, allow Render to connect — `0.0.0.0/0` is the
...
turns defaults are baked into `render.yaml`.
